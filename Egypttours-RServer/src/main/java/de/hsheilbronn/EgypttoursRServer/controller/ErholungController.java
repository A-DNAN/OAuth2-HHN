...
@RestController
@RequestMapping("/erholung")
@CrossOrigin(origins = {"https://seserver.se.hs-heilbronn.de:9443","http://localhost:8081"})
public class ErholungController {

    @Autowired
    @Qualifier("ErholungService")
    IErholungService erholungService;

    @GetMapping("/all")
    public ResponseEntity<Page<ErholungDTO>> getErholungs (
            @RequestParam("page") Integer page,
            @RequestParam(value = "size", required = false) Integer size
    ) {

        Page<ErholungDTO> erholungs = null;
        try {
            erholungs =  erholungService.findAll(page, size);
        ...
        return ResponseEntity.ok().body(erholungs);
    }
